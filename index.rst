...
- **Book**: `Mining the Social Web <http://MiningTheSocialWeb.com>`_ is an
  open source data science project and `book <http://amzn.to/GPd59m>`_ that
  features nearly 130 examples with IPython Notebook and a Vagrant-powered
  virtual machine environment. You can preview all of the example notebooks
  from its GitHub `repository <http://bit.ly/MiningTheSocialWeb2E>`_ on
  IPython's Notebook Viewer
  `here <http://nbviewer.ipython.org/github/ptwobrussell/Mining-the-Social-Web-2nd-Edition/tree/master/ipynb/>`_.
